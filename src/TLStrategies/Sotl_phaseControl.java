 ...
 */
public class Sotl_phaseControl implements TLStrategy, Serializable {

    private static int greenTime = 7500;
    private static int maxGreenTime = 25000;
                ...
                if (trafficLights[i][0].getLightStatus() == TrafficLightStatus.GREEN) {
                    for (int j = 1; j < trafficLights.length; j++) {
                        ...
                        if (trafficLights[next] != null && i != next) {
                            if ((currentTime - greenTime > 0 && thresholdReached(threshold, roads[next]) ) || currentTime - maxGreenTime > 0) {
                                currentTime = 0;
                                setLights(trafficLights[i], TrafficLightStatus.ORANGE, roads[i], tlJunction);
                                //System.out.println("next phase is " + next);
                                nextPhase = next;
                            }
                        }
                    }
    ...
    private int countCarsOnRoad(Road road) {
        return road.getNoOfCarsNearEnd(50);
    }
