 ...
 */
public class Sotl_platoonControl implements TLStrategy, Serializable {
    ...
    // light tries to change to a road with at least this many vehicles
    private int threshold = 30;
    // platoon will be stopped if there are more than this many vehicles incoming on the green light
    private int platoonThreshold = 20;
                ...
                if (trafficLights[i][0].getLightStatus() == TrafficLightStatus.GREEN) {
                    for (int j = 1; j < trafficLights.length; j++) {
                        ...
                        if (trafficLights[next] != null && i != next) {
                            if ((currentTime - greenTime > 0 && thresholdReached(threshold, roads[next]) && (!carNearIntersection(20, roads[i]) && !thresholdReached(platoonThreshold, roads[i]) )) || currentTime - maxGreenTime > 0) {
                                currentTime = 0;
    ...
    private boolean carNearIntersection(int distance, Road road){
        for (Lane lane : road.getLanes()){
            if (lane.getCars().size() > 0 && lane.getCar(0).getPosition() + lane.getCar(0).getLength() > lane.getLength() - distance){
                return true;
            }
        }
        return false;
